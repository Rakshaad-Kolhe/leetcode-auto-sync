...
from release import TARGET_VERSION, compute_sha256, generate_release_notes, run_release_automation


def test_compute_sha256(tmp_path: Path):
    sample_file = tmp_path / "sample.txt"
    sample_file.write_text(f"leetcode-auto-sync v{TARGET_VERSION}", encoding="utf-8")
...
def test_generate_release_notes():
    ...
    assert f"v{TARGET_VERSION}" in notes
